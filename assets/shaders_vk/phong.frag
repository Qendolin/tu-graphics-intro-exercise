...
float specular_to_ior(float specular) {
	return 2 / (1 - sqrt(clamp(specular, 0.0, 1.0) * 0.08)) - 1;
}
...
void main()
{
	...
	diffuse += ortho_diffuse(N, -u_directional_light.direction.xyz) * u_directional_light.color.rgb * u_directional_light.color.a;
	vec3 L = u_point_light.position.xyz - in_position;
	...
	vec3 I = vec3(0.0);
	I += u_material_factors.x * ambient;
	I += u_material_factors.y * diffuse;
	I += u_material_factors.z * specular;
	I = mix(I, getCornellBoxReflectionColor(in_position, clampedReflect(normalize(-V), N)), fresnel_schlick(N, V, 1.2));
