...
#pragma endregion
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            auto delta = input->mouseDelta();
            auto viewport = camera->viewportSize;
            glm::vec3 offset = glm::inverse(glm::mat3(camera->viewMatrix)) * glm::vec3(-delta.x / viewport.x, delta.y / viewport.y, 0);
            orbitCenter += offset;
        }

        camera->position = orbitCenter;
